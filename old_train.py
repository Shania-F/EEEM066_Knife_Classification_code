...
## Training the model
def train(train_loader,model,criterion,optimizer,epoch,valid_accuracy,start, writer):
    losses = AverageMeter()
    model.train()
    model.training=True
...
def evaluate(val_loader,model,criterion,epoch,train_loss,start, writer):
    model.cuda()
    model.eval()
    ...
    with torch.no_grad():
        for i, (images,target,fnames) in enumerate(val_loader):
            ...
            writer.add_scalar('loss/val', losses.avg, epoch * len(val_loader) + i)
            print('\r',end='',flush=True)
            message = '%s   %5.1f %6.1f       |      %0.3f     |      %0.3f    | %s' % (\
...
for epoch in range(0,config.epochs):
    ...
    train_metrics = train(train_loader,model,criterion,optimizer,epoch,val_metrics,start, writer)
    val_metrics = evaluate(val_loader,model,criterion,epoch,train_metrics,start, writer)
